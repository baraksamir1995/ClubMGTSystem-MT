...
return new class extends Migration
{
    ...
    {
        // Receiver's bucket now inherits the sender's expiry at the moment
        // of transfer, and is explicitly tagged as source_type='transfer'.
        // The bucket is otherwise immutable — if the sender later renews,
        // the receiver's expiry is unaffected.
        DB::unprepared(<<<'SQL'
...
BEGIN
  ...
  -- Sender's active subscription bucket. Transfers come from the original
  -- subscription only; you can't re-transfer sessions you yourself received.
  SELECT mm.id, mm.plan_id, mm.sessions_remaining, mm.sessions_total,
         mm.allowed_branch_ids, mm.branch_id, mm.end_date, mp.plan_type
  INTO v_src_mm
  ...
  -- Decrement sender: both remaining and total drop, hiding the transferred
  -- amount entirely from the sender's UI (no "sent X" anywhere).
  ...
  -- Receiver bucket: explicit source_type='transfer', expiry copied from
  -- sender at this moment (immutable thereafter).
...
SQL);
    ...
    {
        // Revert handled by re-running the prior migration's CREATE OR REPLACE.
        // Not safe to drop unconditionally as the function may still be in use.
    }
};
