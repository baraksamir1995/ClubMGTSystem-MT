...
return new class extends Migration
{
    ...
    {
        // Multi-bucket transfer: a single send can drain across the sender's
        // eligible buckets in priority order (subscription first, earliest
        // end_date within each group). This unblocks the case where a member
        // has, say, a 21-session subscription plus 5+2 from prior transfers
        // and wants to send all 28 in one go.
        //
        // Receiver gets ONE merged bucket, carrying:
        //   - merged sessions_total / sessions_remaining
        //   - end_date = MIN(end_date) across consumed sources, so the
        //     receiver can never get a longer window than the most-expiring
        //     source contributed
        //   - branch_id / allowed_branch_ids = first consumed bucket's
        //     (matches the priority pick — branch constraints from later
        //     buckets are intentionally not narrowed onto the merged row;
        //     the receiver still inherits the strongest available access)
        //
        // session_transfers gets one row per source bucket consumed, all
        // referencing the same receiver_membership_id, so the audit chain
        // remains accurate.
        DB::unprepared(<<<'SQL'
...
BEGIN
  ...
  -- Pre-check: sum of remaining across eligible buckets must cover the count.
  -- This avoids partially mutating buckets and then failing.
  ...
  -- We need a stable receiver id up front so per-source audit rows can
  -- reference it.
  v_new_mm_id := gen_random_uuid();
  v_transfer_id := gen_random_uuid();

  -- Walk eligible buckets in priority order, debiting each.
  FOR v_bucket IN
  ...
  LOOP
    ...
    -- Track earliest expiry across consumed buckets — receiver inherits this.
    ...
    -- Per-source audit row, all pointing to the same receiver bucket.
    INSERT INTO session_transfers (
      ...
      v_bucket.id, v_new_mm_id, v_take, now()
    );
  ...
  -- Defensive: if we somehow didn't fulfil (race against another request),
...
SQL);
    ...
    {
        // No-op rollback. Reverting to single-bucket would re-introduce the
        // "stuck remainder" UX bug.
    }
};
