...
return new class extends Migration
{
    ...
    {
        // Fix FK ordering in the multi-bucket transfer: insert the merged
        // receiver bucket FIRST, then write the per-source audit rows
        // (session_transfers.receiver_membership_id has an FK that requires
        // the bucket to exist before being referenced).
        DB::unprepared(<<<'SQL'
...
BEGIN
  ...
  -- Phase 1: drain buckets, accumulate (id, count) into parallel arrays.
  -- Receiver bucket is NOT inserted yet; audit rows can't reference it yet.
  FOR v_bucket IN
  ...
  -- Phase 2: create the merged receiver bucket.
  ...
  -- Phase 3: write per-source audit rows now that the receiver row exists.
...
SQL);
    ...
    {
        // No-op rollback — would re-introduce the broken FK ordering.
    }
};
