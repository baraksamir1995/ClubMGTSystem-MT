...
/**
 * Make the session-transfer receiver lookup tolerant of phone number format.
 *
 * Background — members' phones are stored inconsistently: some as the local
 * Egyptian form "01001116106", some as the normalized "+201001116106". The
 * mobile picker (SessionTransferController::lookup) and this PL/pgSQL function
 * both matched the recipient with an EXACT string compare (p.phone = input),
 * so a member saved as "010…" was invisible to the "+20…" the app sends — they
 * simply never appeared in the share-sessions search even though active.
 *
 * Fix: compare the last 10 digits (the Egyptian national significant number) of
 * the digits-only form on BOTH sides. "01001116106", "+201001116106",
 * "00201001116106", and "1001116106" all reduce to "1001116106" and match.
 * The controller lookup gets the same treatment in the same PR so the picker and
 * the atomic transfer agree on who the recipient is.
 *
 * Only the receiver-by-phone lookup changes; all active/deleted/self/gym checks
 * and the bucket-consumption logic are identical to the prior version.
 */
...
BEGIN
  ...
  -- Last 10 digits of the receiver phone the caller passed (format-agnostic).
  v_phone_key := right(regexp_replace(coalesce(p_receiver_phone, ''), '[^0-9]', '', 'g'), 10);

  -- Receiver lookup by phone — profile must be active + non-deleted. Compare on
  -- normalized digits so "010…" / "+2010…" / "002010…" all resolve to the same
  -- person. When a number maps to more than one profile (e.g. a stale duplicate
  -- in a different format), prefer the one that's an active member of this gym.
  SELECT p.id, p.full_name, p.photo_url
  INTO v_receiver_user, v_receiver_name, v_receiver_photo
  FROM profiles p
  WHERE right(regexp_replace(coalesce(p.phone, ''), '[^0-9]', '', 'g'), 10) = v_phone_key
    AND p.is_active = true
    AND p.deleted_at IS NULL
  ORDER BY (
    EXISTS (
      SELECT 1 FROM gym_members gm
      WHERE gm.user_id = p.id AND gm.gym_id = p_gym_id
        AND gm.deleted_at IS NULL AND gm.status = 'active'
    )
  ) DESC
  LIMIT 1;
...
SQL
    ...
    {
        // Restore the prior body: exact phone match (with the active checks
        // from 2026_05_12_170000_transfer_sessions_block_inactive_members).
        DB::unprepared(<<<'SQL'
CREATE OR REPLACE FUNCTION public.transfer_sessions(p_sender_user_id uuid, p_gym_id uuid, p_receiver_phone text, p_count integer)
 RETURNS jsonb
 LANGUAGE plpgsql
