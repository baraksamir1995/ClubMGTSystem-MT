...
return new class extends Migration
{
    ...
    {
        // Arbitrary source-month -> target-month copy, generalising
        // copy_recurring_sessions_to_month (which hardcodes target =
        // source + 1 month). Same pattern-walk and same "target must be
        // empty" guard; the only new behaviour is that the caller picks
        // both endpoints.
        //
        // Guards beyond the original:
        //   - target month may not be in the past (a copy into a month
        //     that has already happened is never intended, and would
        //     silently create sessions nobody can attend)
        //   - source and target must differ (a self-copy can only ever
        //     hit the not-empty guard, so reject it with a clear reason)
        //
        // The 3-arg copy_recurring_sessions_to_month is deliberately left
        // in place and untouched so the existing "copy to next month"
...
DECLARE
  ...
  v_session_date  date;
BEGIN
  -- Copying into a month that has already passed is never intentional.
  ...
  -- A month cannot be copied onto itself.
  ...
  -- and end up duplicating every session. Keyed on gym+branch only (not
  -- the target month) so it also serialises against the next-month copy.
...
SQL);
    ...
    {
        DB::unprepared('DROP FUNCTION IF EXISTS public.copy_recurring_sessions_between_months(uuid, uuid, date, date);');
    }
};
