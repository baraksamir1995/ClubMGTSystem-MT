...
/**
 * "What's New" history for the current dashboard user.
 *
 * Targeting is resolved server-side in clby-api against the caller's
 * gym_id — this proxy only forwards the token and never passes a gym id,
 * so a crafted request cannot widen the audience.
 */
