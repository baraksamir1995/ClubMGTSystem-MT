...
export default function ClassModal({ existing, branches, defaultBranchId, onClose, onSaved }: Props) {
  ...
  const visibleTrainers = (branches.length > 1 && branchId)
    ? allTrainers.filter(tr => tr.branch_ids.length === 0 || tr.branch_ids.includes(branchId))
    : allTrainers;
    ...
    fetch('/api/trainers')
      ...
      .then((data: any) => {
        if (Array.isArray(data?.trainers)) {
          setAllTrainers(
            data.trainers
              .filter((tr: any) => tr.is_active !== false)
              .map((tr: any) => ({ id: tr.id, name: tr.name, branch_ids: tr.branch_ids ?? [] }))
          );
        }
      })
      .catch(() => {});
  ...
  useEffect(() => {
    if (branches.length > 1 && trainerId && branchId) {
      const trainer = allTrainers.find(tr => tr.id === trainerId);
  ...
  const handleAddType = async () => {
    ...
    try {
      ...
      if (!res.ok) { toast.error(data.error ?? t('classModal.failedToAddType')); return; }
  ...
  const handleDeleteType = async (id: string, typeName: string) => {
    ...
    if (!res.ok) { toast.error(t('classModal.failedToDeleteType')); return; }
    setClassTypes(prev => prev.filter(tp => tp.id !== id));
    if (classType === typeName) setClassType(classTypes.find(tp => tp.id !== id)?.name ?? '');
  };
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { toast.error(data.error ?? t('classModal.uploadFailed')); return; }
      setImageUrl(data.url);
    } catch { toast.error(t('classModal.uploadFailed')); }
    finally { setUploading(false); }
  };

  const handleSubmit = async () => {
    if (!name.trim()) { toast.error(t('classModal.classNameRequired')); return; }
    if (branches.length > 1 && !branchId) { toast.error(t('classModal.selectBranch')); return; }
    setSaving(true);
    try {
      const selectedTrainer = allTrainers.find(tr => tr.id === trainerId) ?? null;
      const body = {
        name: name.trim(),
        classType,
      ...
      const data = await res.json();
      if (!res.ok) { toast.error(data.error ?? t('classModal.failedToSave')); return; }
      toast.success(existing ? t('classModal.classUpdated') : t('classModal.classCreated'));
      onSaved({
  ...
  return (
    <Modal open onClose={onClose} size="md">
      ...
      <Modal.Body className="space-y-4">
        ...
        {branches.length > 1 && (
          <div>
            <label className="block text-xs text-fg-muted mb-1.5">{t('classModal.labelBranch')} <span className="text-danger">*</span></label>
        ...
        {/* Name */}
        <div>
          <label className="block text-xs text-fg-muted mb-1.5">{t('classModal.labelClassName')} <span className="text-danger">*</span></label>
          <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Morning Yoga" />
        </div>

        {/* Type */}
        <div>
          <label className="block text-xs text-fg-muted mb-1.5">{t('classModal.labelClassType')}</label>
          <div className="flex flex-wrap gap-2">
            ...
            <div className="flex items-center gap-1">
              <input
                ...
                placeholder={t('classModal.newTypePlaceholder')}
        ...
        <div>
          <label className="block text-xs text-fg-muted mb-1.5">{t('classModal.labelSpecialist')}</label>
          <Select value={trainerId ?? ''} onChange={e => setTrainerId(e.target.value || null)}>
            <option value="">{t('classModal.noSpecialist')}</option>
        ...
        {/* Location */}
        <div>
          <label className="block text-xs text-fg-muted mb-1.5">{t('classModal.labelLocation')}</label>
          <Input value={location} onChange={e => setLocation(e.target.value)} placeholder="e.g. Studio A" />
        </div>

        {/* Color */}
        <div>
          <label className="block text-xs text-fg-muted mb-1.5">{t('classModal.labelColor')}</label>
        ...
        <div>
          <label className="block text-xs text-fg-muted mb-1.5">
            {t('classModal.labelImage')}
            <span className="ms-2 text-fg-faint font-normal">{t('classModal.imageHint')}</span>
          </label>
          ...
          {imageUrl ? (
            ...
            <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}
              ...
              {uploading
                ? <><Loader2 className="w-5 h-5 animate-spin" /><span className="text-xs">{t('classModal.uploading')}</span></>
                : <><ImagePlus className="w-5 h-5" /><span className="text-xs">{t('classModal.clickToUpload')}</span></>}
            </button>
          )}
        </div>
        ...
        {/* Description */}
        <div>
          <label className="block text-xs text-fg-muted mb-1.5">{t('classModal.labelDescription')}</label>
          <Textarea value={description} onChange={e => setDescription(e.target.value)}
            rows={3} placeholder={t('classModal.descriptionPlaceholder')} className="resize-none" />
        </div>
      </Modal.Body>
      ...
      <Modal.Footer>
        ...
        <Button variant="primary" fullWidth onClick={handleSubmit} disabled={!name.trim()} isLoading={saving}>
          {existing ? tc('saveChanges') : t('classModal.createClass')}
        </Button>
      </Modal.Footer>
    </Modal>
  );
