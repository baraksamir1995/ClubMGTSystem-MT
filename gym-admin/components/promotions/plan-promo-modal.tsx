...
export default function PlanPromoModal({ plans, existing, onClose, onSaved }: Props) {
  ...
  return (
    <Modal open onClose={onClose} size="md">
      <Modal.Header>
        <span className="inline-flex items-center gap-2"><Percent className="w-4 h-4 text-brand" /> {existing ? 'Edit Plan Promotion' : 'Set Plan Promotion'}</span>
      </Modal.Header>

      <Modal.Body className="space-y-4">
        {/* Plan picker */}
        <div>
          <label className="block text-xs text-fg-muted mb-1.5">Membership Plan <span className="text-danger">*</span></label>
          {existing ? (
        ...
        {/* Promo price */}
        <div>
          <label className="block text-xs text-fg-muted mb-1.5">Promotional Price <span className="text-danger">*</span></label>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-fg-muted text-sm pointer-events-none z-10">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-xs text-fg-muted mb-1.5">Valid From <span className="text-danger">*</span></label>
            <Input type="date" value={validFrom} onChange={e => setValidFrom(e.target.value)} className="[color-scheme:dark]" />
          </div>
          <div>
            <label className="block text-xs text-fg-muted mb-1.5">Valid Until <span className="text-danger">*</span></label>
      ...
      <Modal.Footer>
        ...
        <Button variant="primary" fullWidth onClick={handleSave} disabled={!planId || !promoPrice || !validFrom || !validUntil} isLoading={saving}>
          {existing ? 'Save Changes' : 'Set Promotion'}
        </Button>
      </Modal.Footer>
    </Modal>
  );
