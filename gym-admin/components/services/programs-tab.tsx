...
const inp = 'w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 transition-colors';
    ...
    <>
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-gray-400">{programs.length} program{programs.length !== 1 ? 's' : ''}</p>
        ...
        {programs.length === 0 ? (
          <div className="bg-gray-800 border border-gray-700 rounded-xl p-12 text-center">
            ...
            <p className="text-sm text-gray-400">No programs yet. Create your first program.</p>
            {can(permissions, 'services', 'create') && (
              <button onClick={openCreate}
                    ...
                    <tr key={p.id} className="hover:bg-gray-700/30 transition-colors">
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          <div className="w-14 h-14 rounded-lg bg-gray-700 flex-shrink-0 overflow-hidden flex items-center justify-center">
                            {p.image_url ? (
                      ...
                      <td className="px-5 py-3.5">
                        <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${STATUS_STYLES[p.status] ?? 'bg-gray-400/10 text-gray-400'}`}>
                          {p.status}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-gray-800 border border-gray-700 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
              <h2 className="text-base font-semibold text-white">{editingId ? 'Edit Program' : 'New Program'}</h2>
              <button onClick={closeModal} className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-gray-700 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                  ...
                  {(['draft', 'published'] as const).map(s => (
                    <button key={s} type="button" onClick={() => setForm(f => ({ ...f, status: s }))}
                      className={`px-3 py-2.5 rounded-lg border text-left transition-colors ${
                        form.status === s ? 'border-purple-500 bg-purple-600/10' : 'border-gray-700 hover:border-gray-600'
                      }`}>
                      <span className="text-sm font-medium text-white capitalize">{s}</span>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={`text-xs ${form.description.length > 500 ? 'text-red-400' : 'text-gray-500'}`}>
              ...
              {/* Duration */}
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">Duration (weeks)</label>
                ...
                {imageUrl ? (
                  <div className="relative rounded-xl overflow-hidden bg-gray-900 border border-gray-700">
                    ...
                    <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
                      className="absolute bottom-2 right-2 flex items-center gap-1.5 px-3 py-1.5 bg-black/60 hover:bg-black/80 text-white text-xs rounded-lg disabled:opacity-50 transition-colors">
                      <Upload className="w-3 h-3" />{uploading ? 'Uploading…' : 'Replace'}
                    </button>
                  </div>
                ) : (
                  <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
                    className="w-full flex flex-col items-center justify-center gap-2 h-28 bg-gray-900 border border-dashed border-gray-700 hover:border-purple-500 rounded-xl text-gray-500 hover:text-purple-400 disabled:opacity-50 transition-colors">
