              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-line text-xs text-fg-muted uppercase tracking-wide">
                    ...
                    <th className="text-start px-5 py-3">{t('programModal.fieldDuration')}</th>
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex items-center justify-end gap-1">
                          {can(permissions, 'services', 'edit') && (
                            <button onClick={() => openEdit(p)} title={tc('edit')}
                          ...
                          {can(permissions, 'services', 'delete') && (
                            <button onClick={() => deleteProgram(p)} disabled={deletingId === p.id} title={tc('delete')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-surface-2 border border-line rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
              ...
              {/* Title */}
              <div>
                <label className="block text-xs font-medium text-fg-muted mb-1.5">{t('programModal.fieldTitle')} <span className="text-red-400">*</span></label>
                ...
                {imageUrl ? (
                  <div className="relative rounded-xl overflow-hidden bg-surface border border-line">
                    ...
                    <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
                      className="absolute bottom-2 end-2 flex items-center gap-1.5 px-3 py-1.5 bg-black/60 hover:bg-black/80 text-fg text-xs rounded-lg disabled:opacity-50 transition-colors">
                      <Upload className="w-3 h-3" />{uploading ? t('programModal.uploading') : t('programModal.replaceBtn')}
                    </button>
                  </div>
                ) : (
                  <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
